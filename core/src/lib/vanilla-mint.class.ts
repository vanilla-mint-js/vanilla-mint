...
export abstract class VanillaMint<TAttributes> extends HTMLElement {
    static registrar = (mint: CustomElementConstructor & {tagName: string}) => customElements.define(mint.tagName, mint);
    ...
    connectedCallback() {
      (this.vmSubscribe() || []).forEach($ => this.subscriptions.push($.subscribe()));
